...
void Nbjets_Njet_predictor() {
	...
	TH2D *njets_bjets_Z_loose_total_ttjets_predicted = (TH2D*) njets_bjets_Z_loose_total_ttjets->Clone();
	TH2D *njets_bjets_Z_loose_total_ttjets_actual_predicted_ratio = (TH2D*) njets_bjets_Z_loose_total_ttjets->Clone();
	...
	njets_bjets_Z_loose_total_ttjets_predicted->SetName("njets_bjets_Z_loose_total_ttjets_predicted");
	njets_bjets_Z_loose_total_ttjets_predicted->SetTitle("MC ttjets : Total Nr. of Predicted Events vs Jet and BJet Multiplicity for 76 < M(ll) < 106");
	njets_bjets_Z_loose_total_ttjets_actual_predicted_ratio->SetName("njets_bjets_Z_loose_total_ttjets_actual_predicted_ratio");
	njets_bjets_Z_loose_total_ttjets_actual_predicted_ratio->SetTitle("MC ttjets : Ratio of Actual / Predicted Events vs Jet and BJet Multiplicity for 76 < M(ll) < 106");

	// Reading the 2D histogram of NBtags vs NJets
	...
	for(int i=1; i<=13; i++){
		...
		for(int j=1; j<=7; j++){
			events.push_back(njets_btags_Z_loose_total_ttjets->GetBinContent(i,j));
		}
		for(int j=7; j<=14; j++)
			events.push_back(0);
		NB.push_back(events);
	...
	*/
	...
	// Calculating coefficiencts for every bin and calculating the histogram NBjets vs NJets

	std::vector<int> b;

	for(unsigned int u=0; u<NB.size(); u++){

		int njets = u+2;
		std::vector<double> nbjets_njets_events;

		TMatrix nbtags_col(njets+1,1);
		TMatrix nbjets_col(njets+1,1);
		TMatrix coefficients(njets+1,njets+1);
		TMatrix coefficients_inverse(njets+1,njets+1);
		...
		for(unsigned int i=0; i<=njets; i++){
			...
			for(unsigned int j=0; j<=njets; j++){
				...
				nbtags_col(j,0) = NB[u][j];
				// Storing the coefficients in a matrix
				coefficients(i,j) = coeff_calc( tagged_list, untagged_list, b );
			}
		}

		// Inverting coefficient matrix to solve the simultaneous equations
		coefficients_inverse = coefficients.Invert();

		// Solving the simultaneous equations
		nbjets_col.Mult(coefficients_inverse, nbtags_col);

		for(unsigned int x=0; x<=njets; x++){
			nbjets_njets_events.push_back(nbjets_col(x,0));
		}

		for(unsigned int i=njets+1; i<=14; i++){
			nbjets_njets_events.push_back(0);
		}
		NA.push_back(nbjets_njets_events);
	}
    ...
    */
	...
	for(int i=1; i<=13; i++){
		for(int j=1; j<=7; j++){
			njets_bjets_Z_loose_total_ttjets_predicted->SetBinContent(i,j,NA[i-1][j-1]);
		}
	}

	njets_bjets_Z_loose_total_ttjets_actual_predicted_ratio->Divide(njets_bjets_Z_loose_total_ttjets_predicted);
	...
	njets_bjets_Z_loose_total_ttjets_predicted->SetStats(0);
	njets_bjets_Z_loose_total_ttjets_predicted->GetXaxis()->SetNdivisions(13);
	njets_bjets_Z_loose_total_ttjets_predicted->Draw("TEXTCOLZ");
	c3->Print("2D_loose_Z_total_njets_nbjets_predicted_ttjets.pdf");
	delete c3;

	TCanvas *c4 = new TCanvas("c4","test",1100,650);
	njets_bjets_Z_loose_total_ttjets_actual_predicted_ratio->SetStats(0);
	njets_bjets_Z_loose_total_ttjets_actual_predicted_ratio->GetXaxis()->SetNdivisions(13);
	njets_bjets_Z_loose_total_ttjets_actual_predicted_ratio->Draw("TEXTCOLZ");
	c4->Print("2D_loose_Z_total_njets_nbjets_actual_predicted_ratio_ttjets.pdf");
	delete c4;
